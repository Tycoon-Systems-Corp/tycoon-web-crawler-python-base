    ...
    try:
        if product_likely.get('type'):
            if product_likely["type"] == 'og':
                print('og')
                el_title = object_info["product"]["title"] = page.query_selector('meta[property="og:title"]')
                if el_title:
                    object_info["product"]["title"] = el_title.get_attribute("content")
                el_desc = page.query_selector('meta[property="og:description"]')
                if el_desc:
                    object_info["product"]["description"] = el_desc.get_attribute("content")
                ...
                for tag in page.query_selector_all("script"): 
                    ...
                    if tag_type == 'application/ld+json':
                        try:
                            ...
                            content_data = json.loads(content)
                            if content_data.get('@type') and content_data['@type'] == 'Product':
                                schema_urls_match = [ 'http://schema.org/', 'https://schema.org' ]
                                if content_data.get('@context') and content_data['@context'] in schema_urls_match:
                                    if content_data.get('name'):
                                        object_info["product"]["title"] = content_data['name']
                                    if content_data.get('description'):
                                        object_info["product"]["description"] = content_data['description']
                                    if content_data.get('image') and isinstance(content_data['image'], list):
                                        object_info["images"] = content_data['image']
                                    object_info['price'] = get_price(page, 'application/ld+json', content_data)
                                    if content_data.get('brand'):
                                        brand_data = content_data['brand']
                                        if brand_data.get('@type') and brand_data.get('name'):
                                            object_info['product']['brand'] = brand_data['name']
                            return object_info
                        except Exception as e:
